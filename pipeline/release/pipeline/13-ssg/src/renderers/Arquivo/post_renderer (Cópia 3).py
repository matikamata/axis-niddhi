...
import hashlib
import json
import unicodedata                                        # [20260220] adicionado — Step 1: Unicode normalize
...
def resolve_csl_root(start_path: Path) -> Path:
    ...
    current = start_path.resolve().absolute()             # [20260220] .resolve() adicionado — Step 2: hardening contra symlinks
...
def load_glossary(csl_root: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        # [20260220] NFC normalize aplicado nas keys — Step 1: garante match correto
        # de diacríticos Pāli (ex: Ā pode vir como U+0100 ou A + macron combinante)
        glossary = {}
        for key, val in raw_data.items():
            normalized_key = unicodedata.normalize("NFC", key.lower())  # [20260220]
            glossary[normalized_key] = val
...
def inject_marginalia(html_content: str, glossary: Dict[str, Any]) -> str:
    ...
    # [20260220] NFC normalize no conteúdo HTML antes de qualquer match — Step 1.
    # Garante que termos como Tipiṭaka, Pālī, Āsava sejam encontrados mesmo
    # que o HTML venha com normalização diferente do glossary.core.json.
    html_content = unicodedata.normalize("NFC", html_content)           # [20260220]
    ...
    def replacer(match):
        ...
        if term:
            # [20260220] NFC normalize no termo capturado antes do lookup no glossário
            key = unicodedata.normalize("NFC", term.lower())            # [20260220]
