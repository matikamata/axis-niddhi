...
def extract_csl_tattoo(html_content: str) -> str:
    """
    [FF-016] Extrai o comentário canônico CSL do content.html e retorna
    como bloco HTML .csl-tattoo — oculto por padrão, visível no stardust.

    O comentário tem formato:
        <!-- Canonical CSL Artifact\nPD#PN: ...\nExtraction: ...\nIntegrity: ... -->

    Se ausente, retorna string vazia (sem efeito no render).
    """
    match = re.search(
...
def render_post(
    ...
    content_en = link_resolver.resolve_links(raw_en, post.pdpn)
    content_en = process_assets(content_en, post.pdpn, asset_map)
    content_en = inject_marginalia(content_en, glossary)
    # [FF-016] Prepend CSL tattoo block (hidden by default, visible in stardust)
    _tattoo = extract_csl_tattoo(raw_en)
    if _tattoo:
