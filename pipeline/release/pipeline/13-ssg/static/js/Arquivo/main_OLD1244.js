/**
 * BRASILEIRINHO ENGINE — UNIFIED FRONTEND LOGIC V1.3
 */
...
(function () {
  ...
  function initLang() {
    ...
    radios.forEach(r => {
      r.addEventListener('change', () => {
        if (r.checked) localStorage.setItem(LANG_KEY, r.id.replace('lang-', ''));
      });
    });
  }
  ...
  function initAccordion() {
    ...
    sections.forEach(section => {
      ...
      list.style.display = 'none'; // Estado inicial fechado

      h2.onclick = (e) => {
        const isOpen = list.style.display === 'block';
        list.style.display = isOpen ? 'none' : 'block';
        h2.classList.toggle('active', !isOpen);
      };
    });
  }
  ...
  function generateTOC(contentId, listId) {
    ...
    if (!h.length) { list.parentElement.style.display = 'none'; return; }
    h.forEach((el, i) => {
