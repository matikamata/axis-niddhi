...
// [FF-012] View Mode Toggle — global so onclick="" in HTML can reach it
function setViewMode(mode) {
  ...
  const btnComp     = document.getElementById('btn-comparative');
  if (!reader || !comparative) return;

  if (mode === 'comparative') {
    reader.hidden      = true;
    comparative.hidden = false;
    btnReader  && btnReader.classList.remove('active');
    btnComp    && btnComp.classList.add('active');
  } else {
    reader.hidden      = false;
    comparative.hidden = true;
    btnReader  && btnReader.classList.add('active');
    btnComp    && btnComp.classList.remove('active');
  }
