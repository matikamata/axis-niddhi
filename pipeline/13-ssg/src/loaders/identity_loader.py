...
def load_identity(json_path: Path, post_dir: Path) -> Optional[Post]:
    ...
    try:
        ...
        # 3. Metadata Healing (Showcase Polish)
        titles = data.get("titles", {})
        if "pt-BR" in artifacts_map and not titles.get("pt"):
            # Content exists but PT title is null in metadata
            # Heal it using the English title as placeholder
            titles["pt"] = titles.get("en", identity["pdpn"])
            logger.info(f"✨ HEALED: PT title missing for {identity['pdpn']} (Source file found).")

        # 4. Construct Immutable Post Object
        return Post(
            pdpn=identity["pdpn"],
