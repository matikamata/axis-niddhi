#!/usr/bin/env python3
# /beng-fut/pipeline/scripts/SP10_translate_deepl.py
"""
💎 BRASILEIRINHO ENGINE — SP10_translate_deepl.py
===================================================
Versão:  V5.2.3 — AXIS-NIDDHI CLS Integration
Data:    2026-03-08

DELTA vs V5.1:
  ★ [CLS] Após escrita confirmada de content.html PT, chama
          append_translation_event() registrando evento "translated"
          em lineage.translations[] com source_hash e result_hash
  ★ [CLS] graceful degradation — funciona sem cls_tools se necessário

HARDENING V5.1 (mantido):
  ...
  ★ [ND-02]  Timestamp capturado UMA VEZ por post
...
def translate_text(text: str, auth_key: str, glossary_id: str) -> str:
    ...
    """
    if auth_key == "MOCK_KALYANAMITTA_KEY":
        # Nolan Protocol: Retorna o mesmo HTML no modo Mock sem consumir cotas
        return text
...
def resolve_glossary_id(auth_key: str, name_fragment: str = "Glossario_v5") -> Optional[str]:
    """
    Busca o ID do glossário ativo na conta DeepL pelo nome.
    Retorna o ID do primeiro match (case-insensitive) ou None se não encontrado.
    Nunca usa IDs hardcoded — sempre reflete o estado atual da conta.
    """
    if auth_key == "MOCK_KALYANAMITTA_KEY":
        return "mock_glossary_v5"

    resp = requests.get(
        f"{DEEPL_API_URL}/glossaries",
        headers={"Authorization": f"DeepL-Auth-Key {auth_key}"},
        timeout=15,
    )
    if resp.status_code != 200:
        return None
    glossaries = resp.json().get("glossaries", [])
    for g in glossaries:
        if name_fragment.lower() in g.get("name", "").lower():
            return g["glossary_id"]
    # Fallback: retornar o mais recente se houver algum EN→PT-BR
    for g in glossaries:
...
def process_post(
    ...
    try:
        ...
        if not dry_run:
            ...
            # ── [CLS V5.2.3] Registrar evento de tradução ────────────────
            # Chamado APÓS verificação do hash — só registra escrita confirmada
            if _CLS_AVAILABLE:
                try:
                    hash_en = sha256_file(source_en)
                    append_translation_event(
                        identity_path=json_path,
                        event="translated",
                        engine="SP10_v5.2.3",
                        lang="pt-BR",
                        source_hash=hash_en,
                        result_hash=actual_hash,
                        deepl_chars=len(body_en),
                        glossary_id=glossary_id if glossary_id else None,
                    )
                except Exception as cls_err:
                    log(f"   {pdpn}: [CLS] aviso ao registrar evento — {cls_err}", "WARN", YELLOW)
...
def run_translation():
    ...
    try:
        ...
        if total_chars > remaining:
            log(f"⚠️  SALDO INSUFICIENTE NA ESTIMATIVA: precisa {total_chars:,}, tem {remaining:,}.", "WARN", YELLOW)
            log("   O script pode parar no meio do lote se a API negar o serviço.", "WARN", GRAY)
            
    except Exception as e:
        log(f"Aviso: não foi possível verificar saldo DeepL: {e}", "WARN", YELLOW)
        # Se falhar a verificação de saldo, mantemos o total_chars do lote para a confirmação
        total_chars = sum(int(r.get("Chars", 0)) for r in batch)

    # ── Confirmação humana (bypass via env BENG_AUTO_TRANSLATE) ──────────
    auto = os.environ.get("BENG_AUTO_TRANSLATE", "").lower() == "true"
    if not auto:
        # Se o saldo for insuficiente, destacamos em Vermelho
        is_over = total_chars > remaining if remaining > 0 else False
        color_cost = RED if is_over else YELLOW
        
        print(f"\n{color_cost}⚠️  Prestes a traduzir {len(batch)} post(s).")
        print(f"    Custo estimado: {total_chars:,} chars{RESET}")
        
        confirm = input(f"\n{CYAN}Deseja prosseguir com o risco de estourar a cota? [y/N]: {RESET}").strip().lower()
        if confirm != "y":
            log("Tradução cancelada pelo operador.", "WARN", YELLOW)
            flush()
            return
        
        log("Iniciando processamento (Resilience Mode ativo)...", "INFO", CYAN)

    # ── Resolver glossário dinamicamente ─────────────────────────────────
    glossary_id = resolve_glossary_id(auth_key)
    if glossary_id:
        log(f"Glossário DeepL: {glossary_id} (encontrado na conta)", "INFO")
    else:
        log("Glossário DeepL não encontrado na conta — tradução sem glossário.", "WARN", YELLOW)
    ...
    for item in batch:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                result = process_post(item, auth_key, glossary_id, log)
                break
            except Exception as e:
