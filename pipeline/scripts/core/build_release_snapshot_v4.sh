...
info "Copying 28 CORE scripts from scripts/"
# NOTE: models.py and build.py are NOT in scripts/ — they live in 13-ssg/.
# They are copied in Section 7 along with the full SSG engine tree.
...
[[ "$MISSING" -gt 0 ]] && warn "$MISSING scripts were absent in source"
ok "$COPIED/${#CORE_SCRIPTS[@]} scripts from scripts/ copied"

# ==============================================================================
# 7. COPY CLI ENTRY POINTS + FULL SSG ENGINE TREE
# ==============================================================================
# The SSG engine lives entirely under 13-ssg/, NOT under scripts/.
# Real filesystem layout (confirmed from live run):
#   /beng-fut/pipeline/13-ssg/
#   ├── build.py                              ← SD03 engine entry point
#   ├── src/
#   │   ├── models.py                         ← data model (imported by build.py)
#   │   ├── loaders/
#   │   │   ├── csl_loader.py
#   │   │   └── identity_loader.py
#   │   ├── renderers/
...
#   └── static/
#       ├── css/style.css
#       └── js/main.js, sw.js, reading-flow.js
#
# The release copies this entire subtree to pipeline/13-ssg/ and also copies
# all .py modules to scripts/ so that setup_v54_static_site.sh can find and
# deploy them without depending on 13-ssg/ being pre-populated.
# ==============================================================================

SRC_SSG="$SRC_PIPELINE/13-ssg"

info "Copying CLI entry points"
for cli in "axis_cli.sh" "verify_pipeline_integrity.sh" "setup_v54_static_site.sh"; do
    src="$SRC_SCRIPTS/$cli"
    [[ -f "$src" ]] && copy_file "$src" "$REL_SCRIPTS/$cli" || warn "Not found in scripts/: $cli"
done

# ── build.py: copy to both 13-ssg/ (runtime) and scripts/ (builder reference)
info "Copying SSG engine: build.py + models.py"
if [[ -f "$SRC_SSG/build.py" ]]; then
    copy_file "$SRC_SSG/build.py" "$REL_PIPELINE/13-ssg/build.py"
    copy_file "$SRC_SSG/build.py" "$REL_SCRIPTS/build.py"
    ok "build.py (13-ssg/ + scripts/)"
else
    warn "build.py NOT FOUND at $SRC_SSG/build.py — SD phase will fail"
fi

# ── SD03 shim: must also exist in 13-ssg/ for the bootstrap guard
if [[ -f "$SRC_SSG/SD03_static_site_build.py" ]]; then
    copy_file "$SRC_SSG/SD03_static_site_build.py" "$REL_PIPELINE/13-ssg/SD03_static_site_build.py"
else
    # Fallback: use the scripts/ copy (the shim)
    if [[ -f "$REL_SCRIPTS/SD03_static_site_build.py" ]]; then
        copy_file "$REL_SCRIPTS/SD03_static_site_build.py" "$REL_PIPELINE/13-ssg/SD03_static_site_build.py"
        ok "SD03_static_site_build.py (shim → 13-ssg/)"
    fi
fi

# ── models.py: lives in src/ but copied to scripts/ for setup_v54 compatibility
if [[ -f "$SRC_SSG/src/models.py" ]]; then
    copy_file "$SRC_SSG/src/models.py" "$REL_SCRIPTS/models.py"
    ok "models.py (src/ → scripts/)"
else
    warn "models.py NOT FOUND at $SRC_SSG/src/models.py"
fi

# ── SSG Python modules: copy from their real locations to scripts/ for setup_v54
info "Copying SSG modules to scripts/ (for setup_v54_static_site.sh)"

declare -A SSG_MODULE_PATHS=(
    ["csl_loader.py"]="src/loaders/csl_loader.py"
    ["identity_loader.py"]="src/loaders/identity_loader.py"
    ["post_renderer.py"]="src/renderers/post_renderer.py"
    ["index_renderer.py"]="src/renderers/index_renderer.py"
    ["nav_builder.py"]="src/transformers/nav_builder.py"
    ["link_resolver.py"]="src/transformers/link_resolver.py"
    ["asset_mapper.py"]="src/transformers/asset_mapper.py"
    ["language_router.py"]="src/transformers/language_router.py"
)

for module in "${!SSG_MODULE_PATHS[@]}"; do
    rel_path="${SSG_MODULE_PATHS[$module]}"
    src="$SRC_SSG/$rel_path"
    [[ -f "$src" ]] && copy_file "$src" "$REL_SCRIPTS/$module"                     || warn "Not found: 13-ssg/$rel_path"
done

# ── Templates: real location is 13-ssg/templates/
info "Copying templates to scripts/ (for setup_v54_static_site.sh)"
for tpl in "base.html" "post.html" "index.html"; do
    src="$SRC_SSG/templates/$tpl"
    [[ -f "$src" ]] && copy_file "$src" "$REL_SCRIPTS/$tpl"                     || warn "Not found: 13-ssg/templates/$tpl"
done

# ── Static assets: real locations are 13-ssg/static/css/ and 13-ssg/static/js/
info "Copying static assets to scripts/ (for setup_v54_static_site.sh)"
for css in "style.css"; do
    src="$SRC_SSG/static/css/$css"
    [[ -f "$src" ]] && copy_file "$src" "$REL_SCRIPTS/$css"                     || warn "Not found: 13-ssg/static/css/$css"
done
for js in "main.js" "sw.js"; do
    src="$SRC_SSG/static/js/$js"
    [[ -f "$src" ]] && copy_file "$src" "$REL_SCRIPTS/$js"                     || warn "Not found: 13-ssg/static/js/$js"
done
if [[ -f "$SRC_SSG/static/js/reading-flow.js" ]]; then
    copy_file "$SRC_SSG/static/js/reading-flow.js" "$REL_SCRIPTS/reading-flow.js"
fi

# ── Copy full 13-ssg/ src/ subtree to release 13-ssg/ (preserves package structure)
info "Copying full 13-ssg/src/ package tree to release"
if $DRY_RUN; then
    dryrun "rsync -a $SRC_SSG/src/ → $REL_PIPELINE/13-ssg/src/"
    dryrun "rsync -a $SRC_SSG/templates/ → $REL_PIPELINE/13-ssg/templates/"
    dryrun "rsync -a $SRC_SSG/static/ → $REL_PIPELINE/13-ssg/static/"
else
    if [[ -d "$SRC_SSG/src" ]]; then
        make_dir "$REL_PIPELINE/13-ssg/src"
        cp -r "$SRC_SSG/src/." "$REL_PIPELINE/13-ssg/src/"
        ok "13-ssg/src/ package tree"
    else
        warn "13-ssg/src/ not found — SSG will need manual setup_v54 run"
    fi
    if [[ -d "$SRC_SSG/templates" ]]; then
        make_dir "$REL_PIPELINE/13-ssg/templates"
        cp -r "$SRC_SSG/templates/." "$REL_PIPELINE/13-ssg/templates/"
        ok "13-ssg/templates/"
    fi
    if [[ -d "$SRC_SSG/static" ]]; then
        make_dir "$REL_PIPELINE/13-ssg/static"
        cp -r "$SRC_SSG/static/." "$REL_PIPELINE/13-ssg/static/"
        ok "13-ssg/static/ (css, js)"
    fi
fi
...
if ! $DRY_RUN; then
    ...
    # Generate SHA-256 for every file in the release using RELATIVE paths.
    # cd subshell ensures paths in the manifest are relative to RELEASE_ROOT
    # (e.g. ./pipeline/scripts/SG01_extract_html.py, not /beng-release/...).
    # This makes the manifest location-independent: verification works from
    # any directory the release is copied to (archaeology, USB, remote mount).
    (
        cd "$RELEASE_ROOT"
        find . -type f \
