...
#   axis integrity     → run pre-flight integrity check
...
case "$CMD" in
    ...
    help|*)
        ...
        echo "  axis status        Show pipeline status"
        echo "  axis integrity     Run pre-flight integrity check"
...
if ! $DRY_RUN; then
    ...
    # Run integrity guard against the release
    if [[ -f "$REL_SCRIPTS/verify_pipeline_integrity.sh" ]]; then
        ...
        if BENG_BASE="$REL_PIPELINE" bash "$REL_SCRIPTS/verify_pipeline_integrity.sh" --quiet; then
            ok "Release integrity guard: PASS"
        else
...
if $DRY_RUN; then
    ...
    echo -e "${GREEN}${BOLD}║  ✅ RELEASE SNAPSHOT COMPLETE                             ║${NC}"
fi
echo -e "${GREEN}${BOLD}╠══════════════════════════════════════════════════════════╣${NC}"
...
echo -e "${GREEN}${BOLD}║  4. Check:          axis integrity                         ║${NC}"
